...
@patch('project.process_management.get_secret_client',Mock(return_value='secret_client'))
@patch('project.process_management.kv_get_value_secret',Mock(return_value=('secret')))
...
def test_inspection_process_management_assert_called_sending_users_for_testing(sending_users_for_testing):
    inspection_process_management('department',3)
...
@patch('project.process_management.kv_get_value_secret',Mock(return_value=('client_secret','client_id','tenant_id','application_id')))
@patch('project.process_management.get_access_token',Mock(return_value=''))
def test_inspection_process_management_return_failed():
    assert inspection_process_management('department',3)=='Failed to obtain access token'
...
def test_inspection_process_management_return():
    assert  inspection_process_management('department',3) == "sending_users_for_testing does not succeed"
