...
@patch("project.date_check.convert_datetime_to_date",Mock(return_value=date(2023,1,1)))
@patch("project.date_check.disable_user_by_expiration")
def test_is_past_expiration_by_assert_called_disable_user_by_expiration(disable_user_by_expiration):
    is_past_expiration_by(3,date(2023,1,1),'access_token','user_id','application_id')
    disable_user_by_expiration.assert_called_once_with(date(2023,1,1),'access_token','user_id','application_id')
...
@patch("project.date_check.datetime.timedelta",Mock(return_value=timedelta(seconds=259200)))
def test_adding_days_to_date():
    assert adding_days_to_date(3,date(2023,1,1))==date(2023,1,4)


@patch('project.date_check.datetime.date', Mock(**{"today.return_value": date(2023, 11, 6)}))
...
def test_disable_user_by_expiration_assert_called_update_user(update_user_attribute):
    disable_user_by_expiration(date(2023,1,1),'access_token','user_id','application_id')
    update_user_attribute.assert_called_once_with('access_token', 'user_id', "accountEnabled", "false" )
...
def test_disable_user_by_expiration_assert_called_remove_user(remove_user_attribute):
    disable_user_by_expiration(date(2023,1,1),'access_token','user_id','application_id')
    remove_user_attribute.assert_called_once_with('access_token', 'user_id', 'application_id') 
...
def test_disable_user_by_expiration_return_exception():
    assert disable_user_by_expiration(date(2023,1,1),'access_token','user_id','application_id')==None
